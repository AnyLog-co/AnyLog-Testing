        ...
        if isinstance(response, dict):
            try:
                ...
                if 'err_code' in response and 'err_text' in response:
                    pytest.fail(
                        "Failed to extract results from 'COUNT(DISTINCT(device_name))' (Error Code: %s | Error: %s)" % (
...
def test_count_distinct_uuid(self):
    ...
    """
    headers = {
        'command': 'sql %s format=json and stat=false "SELECT COUNT(DISTINCT(parentelement)) FROM ping_sensor"' %
                   self.configs['dbms'],
        'User-Agent': 'AnyLog/1.23',
        'destination': 'network'
    }
    if self.status is True:
        ...
        if isinstance(response, dict):
            try:
                ...
                if 'err_code' in response and 'err_text' in response:
                    pytest.fail(
                        "Failed to extract results from 'COUNT(DISTINCT(parentelement))' (Error Code: %s | Error: %s)" % (
                            response['err_code'], response['err_text']))
                else:
...
def test_aggregate_values(self):
    ...
    if self.status is True:
        for query in expected:
            headers = {
                'command': 'sql %s format=json and stat=false "SELECT %s FROM ping_sensor"' % (
                self.configs['dbms'], query),
                'User-Agent': 'AnyLog/1.23',
                'destination': 'network'
            }
            response = self.anylog_conn.get(headers=headers)
