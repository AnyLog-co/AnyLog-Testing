...
class TestLitSanLeandroSingleTableQueries:
    ...
    def test_distinct_value(self):
        ...
        """
        headers = {
            'command': 'sql %s format=json and stat=false "SELECT DISTINCT(value) FROM ping_sensor ORDER BY DISTINCT(value)"' % self.configs['dbms'],
            'User-Agent': 'AnyLog/1.23',
            'destination': 'network'
        }
        if self.status is True:
    ...
    def test_distinct_uuid(self):
        ...
        """
        headers = {
            'command': 'sql %s format=json and stat=false "SELECT DISTINCT(parentelement) FROM ping_sensor ORDER BY DISTINCT(parentelement)"' %
                       self.configs['dbms'],
            'User-Agent': 'AnyLog/1.23',
            'destination': 'network'
        }
    ...
    def test_distinct_string(self):
        ...
        """
        headers = {
            'command': 'sql %s format=json and stat=false "SELECT DISTINCT(device_name) FROM ping_sensor ORDER BY DISTINCT(device_name)"' %
                       self.configs['dbms'],
            'User-Agent': 'AnyLog/1.23',
            'destination': 'network'
        }
    ...
    def test_count_distinct_string(self):
        ...
        """
        headers = {
            'command': 'sql %s format=json and stat=false "SELECT COUNT(DISTINCT(device_name)) FROM ping_sensor"' %
                       self.configs['dbms'],
            'User-Agent': 'AnyLog/1.23',
            'destination': 'network'
        }
